...
Give a reader of an analysis bundle (D3 output) a way to explore it interactively instead of only through static CSVs and figures: browse threshold-performance curves per (model, workload) pair, review the hypothesis-test and annotator-agreement summary as recorded by the analysis pipeline, and try a live query against the production semantic-cache decision path at a chosen threshold. The dashboard is a thin, local, offline-capable presentation layer over existing bundle and engine contracts — it loads and displays, it does not recompute statistics or reimplement cache-decision logic — and its core logic is testable outside the UI framework so the branch-coverage gate stays green.
