...
Make the declared test tooling real and enforceable: pytest as the canonical runner over the `levy` package, an enforced branch-coverage gate that fails the run below the threshold, explicitly declared exclusions limited to provider internals that cannot be exercised without a network service, and a suite fast and deterministic enough to run offline on every change.
