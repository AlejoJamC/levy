# anthropic-connector Specification
...
Provide the study's real LLM backend: a synchronous `LLMClient` implementation over the official Anthropic SDK, selectable by configuration, that delegates retry to the SDK's own exponential backoff, records real per-call token usage, halts before sending once a configured spend cap is reached, surfaces provider refusals instead of caching empty content, and remains fully testable offline through an injectable HTTP transport.
